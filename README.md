...
| 13:00 МСК **за 2 дня до заседания** | Анонс **всем зарегистрированным**: заседание послезавтра, откройте окно прогнозов |
| 13:00 МСК **накануне заседания** | Напоминание только тем, кто **ещё не сделал прогноз** |
| 13:00 МСК **в день заседания** | Финальное напоминание оставшимся без прогноза (за 20 мин до закрытия окна) |
| 13:20 МСК в день заседания | Окно прогнозов **закрывается** |
...
| Резерв | Администратор запускает рассылку напоминаний вручную: `/send_reminders` (тем, кто без прогноза) |
...
Vercel Cron  → GET  /reminders → api/reminders.py — 13:00 МСК, напоминания (за 2д всем / за 1д и в день — без прогноза)
...
|-----------------|------|-----------|-----------|
| `0 10 * * *` | `/reminders` | 13:00 | Напоминания по расписанию (см. таблицу выше) |
| `30 10 * * *` | `/results` | 13:30 | Получить новую ставку ЦБ, разослать персональные итоги |

**Напоминания отправляются только в активные дни:** за 2 дня, накануне и в день заседания. В остальные дни крон срабатывает, но ничего не рассылает.
