...
- ₿ **Курс криптовалют** — Bitcoin и Ethereum через CoinGecko API
...
- 🔔 **Напоминания** — автоматические уведомления всем зарегистрированным пользователям за 2 дня, накануне и в день заседания ЦБ
...
**Криптовалюты** (BTC, ETH) — через REST API CoinGecko:
- Цена в USD берётся через `https://api.coingecko.com/api/v3/simple/price`
...
| 10:00 МСК **за 2 дня до заседания** | Автоматическое напоминание всем зарегистрированным пользователям без прогноза |
| 13:00 МСК **за 2 дня до заседания** | Второе автоматическое напоминание тем, кто ещё не сделал прогноз |
| 10:00 МСК **накануне заседания** | Автоматическое напоминание всем без прогноза |
| 13:00 МСК **накануне заседания** | Второе напоминание тем, кто ещё не сделал прогноз |
| 10:00 МСК **в день заседания** | Последнее напоминание — окно прогноза закрывается в 13:20 МСК |
| 13:00 МСК **в день заседания** | Финальное напоминание за 20 минут до закрытия окна прогнозов |
| 13:20 МСК в день заседания | Окно прогнозов **закрывается** — новые прогнозы больше не принимаются |
...
| Резерв | Администратор запускает рассылку напоминаний вручную: `/send_reminders` |
| Резерв | Администратор сохраняет ставку и рассылает итоги вручную: `/set_rate 2026-04-25 21.0` |
...
services/
  currency_service.py   — курсы валют (ЦБ РФ + CoinGecko)
...
Vercel Cron  → GET  /reminders → api/reminders.py — 10:00 и 13:00 МСК, напоминания всем зарегистрированным
...
| Расписание (UTC) | Путь | Время МСК | Что делает |
|-----------------|------|-----------|-----------|
| `0 7 * * *` | `/reminders` | 10:00 | Напоминание всем зарегистрированным без прогноза (если до заседания 0–2 дня) |
| `0 10 * * *` | `/reminders` | 13:00 | Второе напоминание тем же пользователям без прогноза |
| `30 10 * * *` | `/results` | 13:30 | Получить новую ставку ЦБ, разослать персональные итоги |

**Напоминания отправляются только в активные дни:** за 2 дня до заседания, накануне и в день заседания. В остальные дни крон срабатывает, но ничего не рассылает.
...
| `/send_reminders` | Разослать напоминания всем зарегистрированным вручную (независимо от того, сколько дней до заседания) |
| `/set_rate 2026-04-25 21.0` | Сохранить ставку и разослать персональные итоги вручную |
